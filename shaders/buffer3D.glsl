...
vec3 rotateZ(vec3 p,float angle){
    mat4 rmy=rotationAxis(angle,vec3(0.,0.,1.));
    return(vec4(p,1.)*rmy).xyz;
}
...
void main(void){
    ...
    float a=sin((u_time*4.)+a_position.y)*.1;
    float b=cos(((u_time+1.5)*10.)+a_position.y*2.)*.05;
    float c=sin((u_time*4.)+a_position.y)*.3;
    v_position.z+=a-b;
    v_position.xyz=rotateZ(v_position.xyz,cos(u_time+a_position.x)*(2.*a));
    // v_position.x -= a + c;
...
void main() {
    ...
    */
    float fresnel=dot(v_normal.xyz,vec3(0.,0.,1.));
    fresnel=1.-pow(fresnel,1.);
    
    // light
    float incidence=max(dot(v_normal.xyz,vec3(0.,1.,0.)),0.);
    vec3 light=vec3(.2)+(vec3(1.)*incidence);
    gl_FragColor=vec4((color.rgb*light)*light,.1+(fresnel+buffer.r)*2.);
